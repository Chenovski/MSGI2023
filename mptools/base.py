  ...
  def train(self,X,Y,initStep=1.,stopTol=1e-4,stopEpochs=5000,alpha=0,plotOpt=True):
    ...
    while not done:
        ...
        for i in np.random.permutation(M):
            ...
            Ji2 = Ji1 + alpha*torch.maximum( self.theta[2]*(self.theta[2]-1), 0*self.theta[2]);
            Ji = Ji2 + alpha*torch.maximum( self.theta[3], 0*self.theta[3]);
        ...
        # stop generating for bad choice of initial values
        if epoch > 3 and J01[-1] > .2:
            display.clear_output(wait=True); 
            print('Warning: bad choice of initial values.')
            break
...
################################################################################
## Base (abstract) "classify" class for multi-classes data   ###################
################################################################################

class jointClassifierMC:
  ...
  def predict(self,X):
    ...
    eps_c = torch.Tensor(X1) @ self.theta[N0:N0+N1]; # CRP binding energy
    eps_r = torch.Tensor(X2) @ self.theta[N0+N1:]; # RNAP binding energy
    wi = self.theta[dN+2]*(torch.exp(-eps_r/RT)+self.theta[dN+1]*torch.exp(-(eps_c+eps_r+self.theta[dN+3])/RT))
    ri = wi/(1+self.theta[dN+1]*torch.exp(-eps_c/RT)+wi)
    
    Y01 = 0
    for i in range(Nc-1):
        Y01 += 1*(ri>self.theta[i])
    Y = self.classes[Y01]           # use lookup to convert back to class values if given
    return Y                        # NOTE: returns as numpy, not torch! (b/c classes is a nparray)
                                    # (This is necessary for mltools plot to work)
  ...
  def train(self,X,Y,initStep=1.,stopTol=1e-4,stopEpochs=5000,alpha=0,plotOpt=True):
    ...
    ----------
    X      : (M,N)
    Y      : numpy.ndarray, binary or multi-class classification
    ...
    # can seal into a function in utils.py
    # can be used with .toIndex, but need to test
    Ytr = [1*(Y>i) for i in range(len(set(Y))-1)]
    
    
    # some consts for future use
    dN = len(Ytr)-1
    N0 = len(Ytr)+3
    N_theta = N0+N # the number of unknown parameters
    ...
    while not done:
        ...
        for i in np.random.permutation(M):
            # Compute predictions and loss for *just* data X[i]:
            # Note: @ operation requires Python 3.5+
            eps_c = torch.tensor(X1[i],dtype=torch.float32) @ self.theta[N0:N0+N1]; # CRP binding energy
            eps_r = torch.tensor(X2[i],dtype=torch.float32) @ self.theta[N0+N1:]; # RNAP binding energy
            wi = self.theta[dN+2]*(torch.exp(-eps_r/RT)+self.theta[dN+1]*torch.exp(-(eps_c+eps_r+self.theta[dN+3])/RT))
            ri = wi/(1+self.theta[dN+1]*torch.exp(-eps_c/RT)+wi)
            
            Ji = 0
            for k,Y01 in enumerate(Ytr):
                si = 1/(1+torch.exp(-(ri-self.theta[k])))
                Ji += -Y01[i]*torch.log(si)-(1-Y01[i])*torch.log(1-si); 
            
            # add penalty to constrain the range of parameters
            for k in range(dN):
                Ji += alpha*torch.maximum( self.theta[k]-self.theta[k+1], 0*self.theta[k]);
                Ji += alpha*torch.maximum( -self.theta[k], 0*self.theta[k]);
            Ji += alpha*torch.maximum( self.theta[dN]-1, 0*self.theta[dN]); 
            for k in range(dN+1,dN+3):
                Ji += alpha*torch.maximum( self.theta[k]*(self.theta[k]-1), 0*self.theta[k]); 
            Ji += alpha*torch.maximum( self.theta[dN+3], 0*self.theta[dN+3]);
